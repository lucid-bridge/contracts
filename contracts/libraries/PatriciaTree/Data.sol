...
library Data {
        ...
        returns (Label memory prefix, Label memory suffix)
    {
        ...
        if (pos == 0) {
            ...
            // prefix.data = bytes32(uint256(self.data) & (~uint256(1) << (255 - pos)));
            prefix.data = self.data & ~bytes32((uint256(1) << (256 - pos)) - 1);
        }
    ...
    */
    ...
    function insertAtEdge(
        ...
        (Label memory prefix, Label memory suffix) = splitCommonPrefix(
            key,
            e.label
        );
        bytes32 newNodeHash;
        if (suffix.length == 0) {
            ...
            (uint256 head, Label memory tail) = chopFirstBit(suffix);
